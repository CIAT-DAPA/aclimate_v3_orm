...
class MngClimateMeasureService(BaseService[MngClimateMeasure]):
    def __init__(self):
        super().__init__(MngClimateMeasure)

    def get_by_name(self, db: Session, name: str, enabled: bool = True) -> List[MngClimateMeasure]:
        return db.query(self.model).filter(self.model.name == name, self.model.enable == enabled).all()

    def get_by_short_name(self, db: Session, short_name: str, enabled: bool = True) -> List[MngClimateMeasure]:
        return db.query(self.model).filter(self.model.short_name == short_name, self.model.enable == enabled).all()

    def get_all(self, db: Session, enabled: bool = True) -> List[MngClimateMeasure]:

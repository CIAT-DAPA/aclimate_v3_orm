...
class ClimateHistoricalDailyService(
    ...
    def get_max_min_by_location_id(self, location_id: int, db: Optional[Session] = None) -> List[Optional[ClimateHistoricalDailyRead]]:
        """
        ...
        with self._session_scope(db) as session:
            ...
            result = []
            if max_record:
                result.append(ClimateHistoricalDailyRead.model_validate(max_record))
            if min_record and (min_record != max_record):
                result.append(ClimateHistoricalDailyRead.model_validate(min_record))
            elif min_record and (min_record == max_record):
                result = [ClimateHistoricalDailyRead.model_validate(max_record)]
            return result

...
from models.administrative import Admin2, Admin1

class Admin2Validator:
    ...
    def validate_admin_1_id(db: Session, admin_1_id: int):
        ...
        admin_1 = db.query(Admin1).filter(Admin1.id == admin_1_id).first()
        if not admin_1:
    ...
    def validate_unique_name(db: Session, name: str, admin_1_id: int):
        ...
        existing_admin2 = db.query(Admin2).filter(Admin2.name == name, Admin2.admin_1_id == admin_1_id).first()
        if existing_admin2:
    ...
    def create_validate(db: Session, obj_in: dict):
        ...
        # Validar el nombre
        Admin2Validator.validate_name(obj_in["name"])

        # Validar que el admin_1_id corresponda a un Admin1 válido
        Admin2Validator.validate_admin_1_id(db, obj_in["admin_1_id"])

        # Validar que el nombre sea único dentro del Admin1
        Admin2Validator.validate_unique_name(db, obj_in["name"], obj_in["admin_1_id"])
